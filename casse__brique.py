...
brique3_x = [1, 225, 1, 225]
brique3_y = [62, 62, 72, 72]
bxx = 0
byy = 0
b3x = 0
b3y = 0
...
def brique_supression():
    ...
    for i in range(0, len(brique_x)):
        if (brique_x[i]+30+2) >= x >= (brique_x[i]-2) and y == (brique_y[i] + 8+2):
            ...
            score += 50
            break
        elif (brique_x[i]+30+2) >= x >= (brique_x[i]-2) and (brique_y[i]-2) > y >= brique_y[i]:
            brique_x.pop(i)
            brique_y.pop(i)
            vitesse_balle_x = vitesse_balle_x
            vitesse_balle_y = -vitesse_balle_y
            score += 50
            break
        elif (brique_x[i]+30+2) >= x >= (brique_x[i]-2) and (brique_y[i]) < y < (brique_y[i] + 8+2):
            brique_x.pop(i)
            brique_y.pop(i)
            vitesse_balle_x = -vitesse_balle_x
            vitesse_balle_y = vitesse_balle_y
...
def brique3_rebond():
    ...
    for h in range(0, len(brique3_x)):
        if (brique3_x[h]+30+rayon) >= x >= (brique3_x[h]-rayon) and y == (brique3_y[h] + 8 + rayon):
            vitesse_balle_x = vitesse_balle_x
            vitesse_balle_y = -vitesse_balle_y
        elif (brique3_x[h]+30+rayon) >= x >= (brique3_x[h]-rayon) and (brique3_y[h]-rayon) <= y <= brique3_y[h]:
            vitesse_balle_x = vitesse_balle_x
            vitesse_balle_y = -vitesse_balle_y
...
def update():
    ...
    global plateforme_x, plateforme_y, balle_liste, x, y, balle, vie, bxx, byy, brique_x, brique_y, brique2_x, brique2_xx, jeu, score
